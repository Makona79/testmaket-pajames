...
if (document.querySelector('.box')) {
	let members = document.querySelectorAll('.box__item');

	for (let index = 0; index < members.length; index++) {
		const member = members[index];
		member.addEventListener("mouseenter", function (e) {
			member.classList.add('_active');
		});
		member.addEventListener("mouseleave", function (e) {
			member.classList.remove('_active');
		});
	}
}

const navItem = document.querySelectorAll('.action__favorite');
...
const addCart = document.querySelectorAll('.action__add');

addCart.forEach(el => {
	el.addEventListener('click', (e) => {
		// e.currentTarget.classList.toggle("_active");
		let addTitle = document.querySelector('.card__title').textContent;
		let value = document.querySelector('.quantity').querySelector('input').value;
		alert('Товар ' + addTitle + ' в количестве ' + value + ' единиц' + ' добавлен в корзину');
	});
});
...
if (document.querySelector('.slider')) {
	let slider_about = new Swiper('.slider__body', {
		...
		// Arrows
		// navigation: {
		// 	nextEl: '.controls-main__arrow_next',
		// 	prevEl: '.controls-main__arrow_prev',
		// },
		/*
		breakpoints: {
			320: {
...
function scroll_scroll() {
	...
	if (src_value > scrollDirection) {
		// downscroll code
		header_wrapper.classList.add('_scrollup');
	} else {
		// upscroll code
		header_wrapper.classList.remove('_scrollup');
	}
